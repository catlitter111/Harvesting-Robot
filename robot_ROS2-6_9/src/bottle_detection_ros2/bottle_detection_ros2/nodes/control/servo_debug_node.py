...
class ServoDebugNode(Node):
    ...
    def send_servo_command(self, servo_id, position):
        ...
        # 构建将要发送给机械臂的实际串口命令格式
        expected_command = f"#{servo_id:03d}P{int(position):04d}T{self.time_ms:04d}!"
        servo_name = "水平舵机" if servo_id == 0 else "垂直舵机"
        
        self.servo_cmd_pub.publish(msg)
        self.get_logger().info(f'[机械臂调试] 发送{servo_name}命令: {expected_command} (ID={servo_id}, 位置={position}PWM, 时间={self.time_ms}ms)')
    ...
    def arm_control_callback(self, msg):
        ...
        try:
            ...
            # 处理机械臂控制命令
            if command == 'arm_up':
                old_pos = self.current_vertical
                self.move_vertical(dynamic_step)  # 正值表示向上
                self.get_logger().info(f'机械臂向上转动，步长: +{dynamic_step}, 位置: {old_pos} → {self.current_vertical}PWM')
            elif command == 'arm_down':
                old_pos = self.current_vertical
                self.move_vertical(-dynamic_step)   # 负值表示向下
                self.get_logger().info(f'机械臂向下转动，步长: -{dynamic_step}, 位置: {old_pos} → {self.current_vertical}PWM')
            elif command == 'arm_left':
                old_pos = self.current_horizontal
                self.move_horizontal(-dynamic_step) # 负值表示向左
                self.get_logger().info(f'机械臂向左转动，步长: -{dynamic_step}, 位置: {old_pos} → {self.current_horizontal}PWM')
            elif command == 'arm_right':
                old_pos = self.current_horizontal
                self.move_horizontal(dynamic_step) # 正值表示向右
                self.get_logger().info(f'机械臂向右转动，步长: +{dynamic_step}, 位置: {old_pos} → {self.current_horizontal}PWM')
                ...
                for servo_id in [0, 1]:  # 停止水平和垂直舵机
                    ...
                    # 构建实际的停止命令格式
                    expected_stop_command = f"#{servo_id:03d}PDST!"
                    servo_name = "水平舵机" if servo_id == 0 else "垂直舵机"
                    
                    self.servo_cmd_pub.publish(stop_msg)
                    self.get_logger().info(f'[机械臂调试] 发送{servo_name}停止命令: {expected_stop_command}')
