...
修复版本的本地控制GUI启动脚本
解决DISPLAY环境变量问题
使用方法: ./start_local_control_gui_fixed.sh
"""
...
# 检查和设置显示环境
echo -e "${BLUE}检查显示环境...${NC}"
if [ -z "$DISPLAY" ]; then
    echo -e "${YELLOW}DISPLAY环境变量未设置，尝试自动设置...${NC}"
    # 尝试几个常见的DISPLAY值
    for display_val in ":0" ":1" ":2" ":10"; do
        export DISPLAY=$display_val
        if xdpyinfo >/dev/null 2>&1; then
            echo -e "${GREEN}找到可用的DISPLAY: $display_val${NC}"
            break
        fi
    done
    ...
    # 如果仍然无法连接显示器
    if ! xdpyinfo >/dev/null 2>&1; then
        echo -e "${RED}错误: 无法连接到任何显示器${NC}"
        echo -e "${YELLOW}请确保：${NC}"
        echo -e "${YELLOW}1. 运行在图形界面环境中${NC}"
        echo -e "${YELLOW}2. DISPLAY环境变量正确设置${NC}"
        echo -e "${YELLOW}3. X11转发已启用（如果使用SSH）${NC}"
        
        # 提供手动设置选项
        read -p "是否要手动设置DISPLAY值? (y/N): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            read -p "请输入DISPLAY值 (例如 :0): " MANUAL_DISPLAY
            export DISPLAY=$MANUAL_DISPLAY
            echo -e "${BLUE}DISPLAY已设置为: $DISPLAY${NC}"
        else
            echo -e "${RED}无法启动GUI，退出${NC}"
            exit 1
        fi
    fi
else
    ...
    # 验证DISPLAY是否可用
    if ! xdpyinfo >/dev/null 2>&1; then
        echo -e "${YELLOW}警告: 当前DISPLAY可能无法使用${NC}"
        echo -e "${YELLOW}尝试其他DISPLAY值...${NC}"
        for display_val in ":0" ":1" ":2" ":10"; do
            export DISPLAY=$display_val
            if xdpyinfo >/dev/null 2>&1; then
                echo -e "${GREEN}切换到可用的DISPLAY: $display_val${NC}"
                break
            fi
        done
    fi
...
python3 -c "import tkinter, PIL, cv2, numpy, rclpy" 2>/dev/null
if [ $? -ne 0 ]; then
...
echo -e "${YELLOW}1) 仅启动GUI界面 (推荐)${NC}"
echo -e "${YELLOW}2) 启动GUI + 基础系统${NC}"
echo -e "${YELLOW}3) 使用修复版launch文件启动${NC}"
echo -e "${YELLOW}4) 直接运行Python模块${NC}"
...
case $CHOICE in
    ...
    2)
        ...
        ;;
    3)
        echo -e "${GREEN}使用修复版launch文件启动...${NC}"
        echo -e "${BLUE}DISPLAY: $DISPLAY${NC}"
        ros2 launch bottle_detection_ros2 local_control_gui.launch.py
        ;;
    4)
        echo -e "${GREEN}直接运行Python模块...${NC}"
        echo -e "${BLUE}DISPLAY: $DISPLAY${NC}"
        cd "$WORKSPACE_DIR"
        python3 -c "
import rclpy
from bottle_detection_ros2.gui.local_control_gui import main
try:
    main()
