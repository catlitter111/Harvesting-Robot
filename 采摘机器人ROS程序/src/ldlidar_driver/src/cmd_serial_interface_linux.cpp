...
CmdSerialInterfaceLinux::CmdSerialInterfaceLinux()
    : rx_thread_(nullptr), rx_count_(0), read_callback_(nullptr) {
  com_handle_ = -1;
  com_baudrate_ = 0;
}

CmdSerialInterfaceLinux::~CmdSerialInterfaceLinux() { Close(); }

bool CmdSerialInterfaceLinux::Open(std::string &port_name, uint32_t com_baudrate) {
  int flags = (O_RDWR | O_NOCTTY | O_NONBLOCK);

  com_handle_ = open(port_name.c_str(), flags);
  if (-1 == com_handle_) {
    LD_LOG_ERROR("CmdSerialInterfaceLinux::Open open error, errno:%d",errno);
    return false;
  }
  ...
  if (ioctl(com_handle_, _IOC(_IOC_READ, 'T', 0x2A, sizeof(struct asmtermios::termios2)), &options)) {
    LD_LOG_ERROR("TCGETS2 first error, errno:%d",errno);
    if (com_handle_ != -1) {
      close(com_handle_);
      com_handle_ = -1;
    }
  ...
  if (ioctl(com_handle_, _IOC(_IOC_WRITE, 'T', 0x2B, sizeof(struct asmtermios::termios2)), &options)) {
    LD_LOG_ERROR("TCSETS2 error, errno:%d",errno);
    if (com_handle_ != -1) {
      close(com_handle_);
      com_handle_ = -1;
    }
  ...
  if (ioctl(com_handle_, _IOC(_IOC_READ, 'T', 0x2A, sizeof(struct asmtermios::termios2)), &options)) {
    LD_LOG_DEBUG("TCGETS2 second error, errno:%d",errno);
    if (com_handle_ != -1) {
      close(com_handle_);
      com_handle_ = -1;
    }
...
bool CmdSerialInterfaceLinux::Close() {
...
bool CmdSerialInterfaceLinux::ReadFromIO(uint8_t *rx_buf, uint32_t rx_buf_len,
                                   uint32_t *rx_len) {
...
bool CmdSerialInterfaceLinux::WriteToIo(const uint8_t *tx_buf, uint32_t tx_buf_len,
                                  uint32_t *tx_len) {
  int32_t len = -1;
...
void CmdSerialInterfaceLinux::RxThreadProc(void *param) {
  CmdSerialInterfaceLinux *cmd_if = (CmdSerialInterfaceLinux *)param;
